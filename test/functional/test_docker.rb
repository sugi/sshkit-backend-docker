  ...
  module Backend
    class TestDocker < Minitest::Test
      ...
      def test_run_image_returns_different_container_id_for_each_image
        cid1 = docker.docker_run_image 'busybox:musl'
        cid2 = docker.docker_run_image 'busybox:glibc'
        cid3 = docker.docker_run_image 'busybox:uclibc'
        assert cid1
        assert cid2
        assert cid3
        assert cid1 != cid2
      ...
      def test_run_image_fails_to_run_missing_image
        assert_raises do
          docker.docker_run_image Host.new(docker: {image: 'do-not-exist-image'})
        end
      end
      ...
      def test_commit
        ...
        ret_commit_id = nil
        Docker.new(Host.new(docker: {image: 'busybox', commit: try_commit_id})) do |_host|
      ...
      def test_commit_without_name
        ret_commit_id = nil
        Docker.new(Host.new(docker: {image: 'busybox', commit: true})) do |_host|
          upload! '/etc/hostname', '/tmp/hostname'
          ret_commit_id = docker_commit
        end.run
        assert ret_commit_id
      ...
      def test_commit_with_options
        try_commit_id = "commit-test-#{$$}-#{rand}"
        ret_commit_id = nil
        Docker.new(Host.new(docker: {image: 'busybox', commit: {author: 'hoge', name: try_commit_id}})) do |_host|
          upload! '/etc/hostname', '/tmp/hostname'
          ret_commit_id = docker_commit
        end.run
        assert ret_commit_id
